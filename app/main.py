...
@app.post(f"{settings.API_V1_STR}/monitor")
...
@app.get(f"{settings.API_V1_STR}/signals", response_model=List[dict])
def get_signals():
    return active_signals

# ✅ Endpoint: получить все сигналы по тикеру
@app.get(f"{settings.API_V1_STR}/signals/{{ticker}}", response_model=List[dict])
...
@app.post(f"{settings.API_V1_STR}/signals/confirm")
...
@app.get(f"{settings.API_V1_STR}/signals/history", response_model=List[dict])
def get_signal_history():
    return signal_history

# ✅ Endpoint: удалить тикер из мониторинга
@app.delete(f"{settings.API_V1_STR}/monitor/{{ticker}}")
...
@app.get(f"{settings.API_V1_STR}/watchlist", response_model=List[str])
def get_watchlist():
    return list(watchlist)

# 🆕 Endpoint: получить текущий баланс
@app.get(f"{settings.API_V1_STR}/balance")
def get_balance():
    return {"balance": user_balance}

# 🆕 Endpoint: получить открытые позиции
@app.get(f"{settings.API_V1_STR}/positions")
def get_positions():
    return {
        "positions": list(open_positions.values()),
        "balance": user_balance
...
@app.get(f"{settings.API_V1_STR}/positions/history")
def get_position_history():
    return position_history

# 🆕 Endpoint: закрыть позицию
@app.post(f"{settings.API_V1_STR}/positions/close/{{ticker}}")
...
@app.post(f"{settings.API_V1_STR}/portfolio/reset")
def reset_portfolio():
